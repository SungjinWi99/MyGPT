# 한국어 사전학습 데이터 준비

현재 파이프라인은 다음 두 소스를 하나의 정규화된 사전학습 데이터셋으로 만든다.

1. 한국어 Wikimedia 공식 덤프
2. `HAERAE-HUB/KOREAN-WEBTEXT`

Open Korean Historical Corpus는 현대 한국어 스모크 후보가 한자·옛한글
중심이었고 레코드의 `copyright`가 비어 있어 현재 빌드에서 제외했다. NIKL도
현재 제외 상태이며, 두 어댑터는 향후 재검토를 위해 코드에 유지한다.

출력은 Google Drive의 버전 디렉터리에 저장된다.

```text
/content/drive/MyDrive/KTB/MyGPT/datasets/
  raw/
    wikimedia/
    webtext/
      <hugging-face-revision>/
  pretrain/
    v1/
      parquet/
        train/
        validation/
      manifest.json
      profile.json
      samples.jsonl
```

## 처리 정책

- 텍스트는 Unicode NFC와 LF 줄바꿈으로 정규화한다.
- 정규화 후 SHA-256이 같은 문서는 선택한 소스 전체에서 한 번만 남긴다.
- 주제, 안전성, 문서 길이 기반 필터는 적용하지 않는다.
- KOREAN-WEBTEXT는 `news`, `science-webtext`, `law` 상류 소스만
  기본 채택한다.
- `cultureY`는 1,000건 스모크 샘플에서 번역 광고, 꿈 해몽, SEO성 레시피
  묶음 등 품질 편차가 확인되어 현재 버전에서 보류한다.
- `oscar2109`, `oscar2201`, `oscar2301`, `cc100` 등 범용 웹 크롤링
  계열은 품질 편차가 커 현재 버전에서 보류한다.
- 성인 서비스·도박 광고, 여러 금융 홍보 문구의 동시 출현, 같은 4단어 구절의
  과도한 반복처럼 명백한 기술적 스팸만 보수적으로 제거한다.
- KOREAN-WEBTEXT는 상류 데이터셋의 문장·문서 필터 및 중복 제거가 이미
  적용된 Parquet을 입력으로 사용한다.
- KOREAN-WEBTEXT 데이터 카드에는 명시적 라이선스가 없으므로 매니페스트에
  `Not declared in the dataset card`로 기록하고 결과물 재배포는 보류한다.
- 문서 해시를 이용해 99.5% 학습, 0.5% 검증으로 결정론적으로 분할한다.
- `skt/kogpt2-base-v2`의 실제 Hub 커밋을 조회해 매니페스트에 고정한다.
- 정제된 원문은 Zstandard 압축 Parquet으로 저장한다.
- 데이터 버전은 불변이다. 이미 `v1`이 있으면 덮어쓰지 않는다.

`profile.json`에는 소스별 문서 수, 제거 사유, UTF-8 용량, KoGPT2 토큰 수,
문서 길이 백분위수, 문자 비율과 채택된 WebText 상류 소스별 문서 수가
기록된다. `samples.jsonl`에는 수동 검토용 결정론적 샘플이 저장된다.

## Colab 설치

```python
from google.colab import drive
drive.mount("/content/drive")
```

```bash
git clone https://github.com/SungjinWi99/MyGPT.git
cd MyGPT
pip install -r requirements.txt
```

이미 clone한 저장소라면 `git pull`만 실행한다.

## 소스별 스모크 테스트

정식 빌드 전에 각 어댑터를 별도 버전으로 확인한다. 출력 버전 이름은 다시
사용하지 않는다.

### Wikimedia

최초 실행 시 약 1.29GB 공식 `pages-articles.xml.bz2` 덤프를
`raw/wikimedia`에 내려받고 Wikimedia가 공개한 SHA1 체크섬을 검증한다.
`multistream` 결합 파일은 공식 `sha1sums.txt`에 체크섬이 없어 기본 입력으로
사용하지 않는다.

```bash
python -m src.dataset_pipeline.prepare_pretrain \
  --output-dir /content/drive/MyDrive/KTB/MyGPT/datasets/pretrain/smoke-wikimedia-v1 \
  --work-dir /content/mygpt_dataset_work \
  --raw-cache-dir /content/drive/MyDrive/KTB/MyGPT/datasets/raw \
  --sources wikimedia \
  --max-accepted-per-source 1000
```

### KOREAN-WEBTEXT

전체 데이터는 18개 Parquet 샤드, 약 4.47GB 다운로드, 데이터 카드 기준
128만 문서와 약 22억 토큰이다.

여기서 `OSCAR`는 Common Crawl 웹 페이지에서 언어별 문서를 자동 추출한
다국어 말뭉치 계열이다. `oscar2109`, `oscar2201`, `oscar2301`은 각각의
릴리스 또는 크롤링 스냅샷을 가리키며 품질 등급이 아니다. 수집 범위가 넓은
만큼 정상 문서와 광고, SEO 문서, 번역체, 반복 문서가 함께 섞일 수 있어 현재
버전에서는 OSCAR와 CC100 계열을 보류한다. OSCAR의 수집·정제 구조는
[OSCAR 논문](https://arxiv.org/abs/2201.06642)에서 확인할 수 있다.

스모크 테스트는 trusted source가 실제 포함된 다섯 번째 샤드만 내려받고
1,000개의 채택 문서를 처리한다. 이 샤드에서 확인된 상류 소스는
`cultureY`, `law`, `news`, `science-webtext`, `oscar2109`이며 기본
allowlist에 따라 `cultureY`와 `oscar2109`는 제외된다.

```bash
python -m src.dataset_pipeline.prepare_pretrain \
  --output-dir /content/drive/MyDrive/KTB/MyGPT/datasets/pretrain/smoke-webtext-trusted-v1 \
  --work-dir /content/mygpt_dataset_work \
  --raw-cache-dir /content/drive/MyDrive/KTB/MyGPT/datasets/raw \
  --sources webtext \
  --webtext-allow-pattern 'data/train-00004-of-00018.parquet' \
  --webtext-upstream-sources news science-webtext law \
  --max-accepted-per-source 1000
```

필요한 비교 실험에서만 `--webtext-upstream-sources ANY`로 출처 필터를
해제하거나 `--webtext-keep-obvious-spam`으로 명백한 스팸 필터를 끈다.
기본 정식 빌드에서는 두 옵션을 해제하지 않는다.

각 실행 후 `profile.json`과 `samples.jsonl`을 먼저 검토한다. 정식 빌드를
시작하기 전에 두 스모크 결과를 Codex와 함께 확인한다.

## 정식 통합 빌드

스모크 결과를 승인한 뒤 실행한다.

```bash
chmod +x scripts/prepare_pretrain_colab.sh
./scripts/prepare_pretrain_colab.sh
```

스크립트 기본값:

```text
DATASET_ROOT=/content/drive/MyDrive/KTB/MyGPT/datasets
OUTPUT_VERSION=v1
WORK_DIR=/content/mygpt_dataset_work
RAW_CACHE_DIR=$DATASET_ROOT/raw
```

필요하면 출력 버전을 환경 변수로 변경할 수 있다.

```bash
OUTPUT_VERSION=v2 ./scripts/prepare_pretrain_colab.sh
```

AIHub 원본을 함께 섞을 때는 Google Drive에 업로드한 원본 폴더를
`AIHUB_ROOT`로 지정하고 `INCLUDE_AIHUB=1`을 켠다. 기본 경로는 사용자가
업로드한 위치인 `/content/drive/MyDrive/KTB/datasets/AIHUB`이다.

```bash
INCLUDE_AIHUB=1 OUTPUT_VERSION=v3 ./scripts/prepare_pretrain_colab.sh
```

스크립트는 AIHub에서 다음 원천 데이터를 기본 pretrain 후보로 읽는다.

- `121.한국어 성능이 개선된 초거대AI 언어모델 개발 및 데이터`의
  `한국어말뭉치데이터` zip: `data_info[*].contents`를 사용한다.
- `048.일반상식 문장 생성 데이터`의 원천 zip:
  `sentence` 필드를 짧은 일반상식 문장 문서로 사용한다.

0바이트 zip과 `.irx*` 조각 파일은 제외한다.
`Validation/01.원천데이터`까지 포함하려면 CLI를 직접 호출하면서
`--aihub-include-validation-source`를 명시한다. RLHF zip은 사전학습 원문
말뭉치가 아니므로 기본에서 제외한다.

## 실패한 빌드

성공한 빌드만 최종 버전 디렉터리로 승격된다. 실행이 중단되면 다음과 같은
미완료 디렉터리가 남을 수 있다.

```text
/content/drive/MyDrive/KTB/MyGPT/datasets/pretrain/.v1.building
/content/mygpt_dataset_work/.<build-id>.building
```

원인을 확인한 뒤 이 미완료 디렉터리를 삭제하고 같은 명령을 다시 실행한다.
원본 다운로드 캐시는 Drive에 남아 있으므로 Wikimedia와 Hugging Face 파일은
이어받거나 재사용된다.

## 데이터 규모와 학습 예산

정식 통합 빌드는 원본 다운로드를 재사용할 수 있도록 Wikimedia와
KOREAN-WEBTEXT 원본 캐시를 Drive에 보존하고, 정규화된 Parquet에는 필터를
통과한 문서만 기록한다. 이것이 8~12시간 동안 상류 데이터의 22억 토큰을 전부
학습한다는 뜻은 아니다. A100 단일 GPU 학습용 토큰 예산은 `config.yaml`의
`data.token_budget`과 `training.max_steps`로 제한하고, `src.data.PackedPretrainDataset`이
정규화된 Parquet을 고정 길이 packed cache로 변환해 학습에 사용한다.

## 랜덤 샘플 검토

준비된 Parquet 데이터셋에서 사람이 직접 읽을 랜덤 샘플을 만들 수 있다.
JSONL에는 전체 본문이 들어가고, HTML은 빠른 검토를 위해 본문을 일부만
미리보기로 보여준다.

```bash
python scripts/sample_pretrain_dataset.py \
  --dataset-path /content/drive/MyDrive/KTB/MyGPT/datasets/pretrain/v2 \
  --output-dir /content/drive/MyDrive/KTB/MyGPT/datasets/pretrain/v2/review_samples \
  --split train \
  --sample-size 10000 \
  --seed 42
```

출력 파일:

```text
pretrain_train_random_10000_seed42.jsonl
pretrain_train_random_10000_seed42.summary.json
pretrain_train_random_10000_seed42.html
```
